import { KWorkspaceItem, FolderItem, NoteItem, FileItem, isFolder, canHaveChildren } from "../types/K.types";
import { KItemV2, WorkspaceFolderItem, isFolder as isFolderV2, canHaveChildren as canHaveChildrenV2 } from "../types/K-v2.types";
 ...
 */
export function $checkSubtree(node: KTreeFolder, targetId: number): boolean {
...
/**
 * Recursive filter KTreeFolder by search text (V2 structure)
 * Includes node if it matches OR any descendant matches
 * Shows matching nodes WITH all their ancestors (parent, grandparent, etc.)
 *
 * Search modes (configured in kconstants.workspace.search.mode):
...
export function $filterTreeBySearchV2(nodes: KTreeFolder[], searchText: string): KTreeFolder[] {
    ...
    for (const node of nodes) {
        ...
        if (matchesSearch || hasMatchingDescendant) {
            ...
            let childrenToInclude: KTreeFolder[];
...
/**
 * Recursive transform KWorkspaceItem to Folder
 * Only transforms FolderItem - notes and files return null
 */
export function $transformItemToFolder(item: KWorkspaceItem): Folder | null {
    if (!isFolder(item)) {
        return null;
    }
 ...
 * Can wrap any KWorkspaceItem (folder, note, or file)
 */
export interface TreeNode {
    id: string;
    name: string;
    data: KWorkspaceItem; // Can be FolderItem | NoteItem | FileItem
    children: TreeNode[];
}

export function isDescendant(targetId: number, potentialParentId: number, treeData: KTreeFolder[]): boolean {
...
export interface KTreeFolder {
    id: string;
    name: string;
    children?: KTreeFolder[];
    data: KWorkspaceItem; // Can be FolderItem | NoteItem | FileItem
}
 ...
 */
export function getAllVisibleFolderIds(treeData: KTreeFolder[]): number[] {
...
/**
 * Transform KWorkspaceItem hierarchy to react-arborist tree data
 ...
 */
export function transformItemsToTreeData(items: KWorkspaceItem[]): KTreeFolder[] {
    return items
...
function $sortChildrenRecursively(nodes: KTreeFolder[]) {
    nodes.forEach(node => {
        if (node.children && node.children.length > 0) {
 ...
 */
function sortTreeNodes(nodes: KTreeFolder[]): KTreeFolder[] {
...
/**
 * Build hierarchical KTreeFolder structure from flat V2 list
 *
 * KEY CONCEPT:
 * - KItemV2.id = workspace_items.id (workspace item ID)
 * - KItemV2.entityId = entity ID (folders.id | notes.id | files.id)
 * - KItemV2.parentId = parent workspace_items.id (SELF-REFERENCING, NOT entity ID!)
 *
 * ALGORITHM:
 ...
 */
export function buildTreeFromV2Items(items: KItemV2[]): KTreeFolder[] {
    // -------------------------------------------------------
    // STEP 1: CREATE MAP FOR O(1) LOOKUP
    // -------------------------------------------------------
    // Map key = workspace_items.id
    // Map value = KTreeFolder node
    // Why workspace_items.id? Because parentId references parent's workspace_items.id (self-referencing)
    const workspaceItemIdToNodeMap = new Map<number, KTreeFolder>();
    const allTreeNodes: KTreeFolder[] = [];

    // Create KTreeFolder node for each workspace item
    items.forEach((workspaceItem) => {
        const treeNode: KTreeFolder = {
 ...
 */
// function buildHierarchy(items: KWorkspaceItem[]): KWorkspaceItem[] {
//     // Create a map for O(1) lookup - include ALL item types
//     const itemMap = new Map<number, KWorkspaceItem>();

//     items.forEach((item) => {
//         // Only folders can have children
//         const itemWithChildren: KWorkspaceItem = isFolder(item) ? { ...item, children: [] } : { ...item, children: [] as any }; // Notes/files will keep empty children

//         itemMap.set(item.id, itemWithChildren);
//     });

//     // Build parent-child relationships
//     const roots: KWorkspaceItem[] = [];
 ...
 * 5. Convert to KTreeFolder format
 *
 ...
 *
 * @param data - KWorkspaceDTO with workspace data + flatData (or null/undefined)
 * @param searchText - Search filter text (filters by name)
 * @returns KTreeFolder array ready for react-arborist
 */
export function transformToTreeData(
    data: KWorkspaceDTO | null | undefined,
    searchText: string,
): KTreeFolder[] {
    // Validate input data
    ...
    // Create workspace root node from KWorkspaceDTO
    const workspaceRootV2: KTreeFolder = {
        id: `${kconstants.workspace.root.workspaceItemId}`,
        name: data.name,
        data: {
            // KItemV2 structure
            id: kconstants.workspace.root.workspaceItemId, // workspace_items.id
            workspaceId: data.id,
            parentId: null,
            entityType: 2, // folder
            entityId: kconstants.workspace.root.entityId, // folders.id (entity ID)
            ...
            data: {
                ...
                id: kconstants.workspace.root.entityId, // folders.id (entity ID)
                userId: data.userId,
                name: data.name,
 ...
 */
export function findPathToItem(treeData: KTreeFolder[], targetId: number): number[] {
    function $findPath(nodes: KTreeFolder[], path: number[]): number[] | null {
        for (const node of nodes) {
...
export async function expandPathToItem(
    ...
    const closeNonPathNodes = (nodes: KTreeFolder[]) => {
