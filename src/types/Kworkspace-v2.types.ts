 ...
 */
export type KWorkspaceItemV2 = WorkspaceFolderItem | WorkspaceNoteItem | WorkspaceFileItem;
...
export function isFolder(item: KWorkspaceItemV2): item is WorkspaceFolderItem {
  return item.entityType === 2;
}

/** Type guard to check if item is a note */
export function isNote(item: KWorkspaceItemV2): item is WorkspaceNoteItem {
  return item.entityType === 3;
}

/** Type guard to check if item is a file */
export function isFile(item: KWorkspaceItemV2): item is WorkspaceFileItem {
  return item.entityType === 4;
}

/** Type guard to check if item can have children (folders only) */
export function canHaveChildren(item: KWorkspaceItemV2): item is WorkspaceFolderItem {
  return item.entityType === 2;
}
