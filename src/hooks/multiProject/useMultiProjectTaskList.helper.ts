...
import { getSubtasksOutsideRange } from "@/utils/task/TaskGrid.utils";
...
export const useMultiProjectTaskListHelper = () => {
    ...
    const handleInlineDateUpdate = useCallback(
        ...
        [$user.userToken, projectIds, tasks, _console],
    );
    ...
    const handleDropTaskOntoTask = useCallback(
        ...
        [$user.userToken, loadTasksForProjects, projectIds, _console],
    );
