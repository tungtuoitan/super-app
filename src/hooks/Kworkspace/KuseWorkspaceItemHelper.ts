...
import { KWorkspaceItemAction, KUpsertWorkspaceItemRequest } from "@/types/Kworkspace.types";
...
export const KuseWorkspaceItemHelper = () => {
    ...
    const upsertWorkspaceItem = useCallback(
        async (action: KWorkspaceItemAction, tabIds?: string[]): Promise<boolean> => {
                ...
                case KWorkspaceItemAction.Create:
                    ...
                    for (const tab of tabsToProcess) {
                        ...
                        const request: KUpsertWorkspaceItemRequest = {
                            action: KWorkspaceItemAction.Create,
                            entityType: 3, // Note
                    ...
                    // =====================================
                    const result = await KWorkspaceService._upsertWorkspaceItems(token ?? "", currentWorkspace?.id ?? 0, batchRequests);
