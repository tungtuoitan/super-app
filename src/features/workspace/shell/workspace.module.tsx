...
import { shellConstants, type ModuleDefinition, useSideBarStore } from "@/shell";
...
export const workspaceModule: ModuleDefinition = {
    ...
    useOnTabClose: () => {
        ...
        return (tab: BaseTab) => {
            ...
            if (moduleName !== "Workspace") return;

            if (tab.type === shellConstants.vscode.tab.tabTypes.note) {
                // entityType 3 = note in workspace tree
                const entityId = (tab.data as { id: number }).id;
                if (entityId < 0) {
    ...
    useOnTabActivate: () => {
        const { currentWorkspace, setSelectedItemIds, _treeRef, setLastSelectedItemId } = useWorkspaceStore();

        return (tab: BaseTab | null) => {
            if (!tab) {
                setSelectedItemIds([]);
        ...
        return (tab: BaseTab) => {
            ...
            try {
                ...
                if (noteData.id < 0) {
                    ...
                    return markDisabled(buildBreadcrumbFromTree(
                ...
                return markDisabled(enrichBreadcrumbWithColors(
                    parseBreadcrumbFromKeyword(keyword),
                    allKeywords,
                ));
            } catch {
                return undefined;
