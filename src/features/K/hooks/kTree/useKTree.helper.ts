...
export const useKTreeHelper = () => {
            ...
            if (virtualItemsNeedUpdate.length > 0) {
                updatedVirtualItems = virtualItemsNeedUpdate
                    .map((workspaceItemId) => {
                        ...
                        // KTreeNode.data is already KItemV2
                ...
                const otherVirtualItems = allNodes.filter((n) => n.data.id < 0 && !SPECIAL_IDS.includes(n.data.id) && !virtualItemsNeedUpdate.includes(n.data.id)).map((node) => node.data);
    ...
    const addNewFolder = (treeData: KTreeNode[]) => {
        ...
        openNodeDialog("create", workspaceConstants.itemTypes.node, null, parentNode ?? null);
    };
