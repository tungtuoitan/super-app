...
import { LibraryBig, Library, Maximize2, Minimize2, Bookmark } from "lucide-react";
...
interface NodeIconDisplayProps {
    ...
    IconComponent: FC<{ className?: string; style?: React.CSSProperties; strokeWidth?: number }> | null;
}

function NodeIconDisplay({ opacity = 0.4, isKnowledge, isRoot, iconColor, nodeColor, IconComponent }: NodeIconDisplayProps) {
    if (isKnowledge) {
        return <LibraryBig className="w-3.5 h-3.5" style={{ color: "#A1887F", opacity }} strokeWidth={2} />;
    }
    if (isRoot) {
        return <LibraryBig className="w-3.5 h-3.5" style={{ color: nodeColor || "#A1887F", opacity }} strokeWidth={2} />;
    }
    if (IconComponent) {
...
export function KNodeCard({ node, isRoot, compact, onSubmitEdit }: { node: KItemV2; isRoot?: boolean; compact?: boolean; onSubmitEdit?: (draft: { name: string; description: string; icon: string | null; color: string | null }) => Promise<void> }) {
    const {
